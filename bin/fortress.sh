...
# Application
APP_NAME="fortress"
APP_LONG_NAME="Hippo Fortress"

# Wrapper
if [ `uname` = 'Darwin' ] ; then
	if [ `uname -p` = 'powerpc' ] ; then
		DEFAULT_WRAPPER_CMD="./wrapper.osx.ppc"
	elif [ `uname -p` = 'i386' ] ; then
		DEFAULT_WRAPPER_CMD="./wrapper.osx.i386"
	else
		echo "Unknown Darwin platform"
		exit 1
	fi
...
DEFAULT_WRAPPER_CONF="./wrapper.conf"
...
# Prevent root user from running server

# Check the configured user
if [ "X$RUN_AS_USER" = "X" ]; then
	if test "$USER" = "root"; then echo "Do not run server as 'root'."; exit; fi
fi

if test "$WRAPPER_CONF" = ""; then WRAPPER_CONF=$DEFAULT_WRAPPER_CONF; fi
if test "$WRAPPER_CMD" = ""; then WRAPPER_CMD=$DEFAULT_WRAPPER_CMD; fi

# Set exec flag on fortress script
if [ ! -x "$0" ]; then
     chmod +x $0
fi
...
# Set exec flag on wrapper command
